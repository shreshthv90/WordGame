...
function App() {
  ...
  const selectLetter = (letterId, letter) => {
    if (selectedLetters.find(l => l.id === letterId)) {
      ...
      setCurrentWord(prev => {
        const newWord = prev.split('').filter((_, index) => {
          const letterAtIndex = selectedLetters.find((_, i) => i === index);
          return letterAtIndex && letterAtIndex.id !== letterId;
        }).join('');
        return newWord;
      });
    } else {
  ...
  const submitWord = () => {
    if (wsRef.current && currentWord.length >= 3 && selectedLetters.length >= 3) {
      wsRef.current.send(JSON.stringify({
        type: 'submit_word',
        word: currentWord,
  ...
  useEffect(() => {
    if (gameState === 'lobby' && wsRef.current && playerName) {
      joinGame();
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-red-50 flex items-center justify-center p-4">
        <Card className="w-full max-w-2xl shadow-2xl border-0 bg-white/95 backdrop-blur-sm">
          <CardHeader className="text-center">
            ...
            <div className="flex items-center justify-center gap-2 text-lg font-mono text-amber-600 bg-amber-50 py-2 px-4 rounded-lg">
      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-red-50 p-4">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <Card className="bg-white/95 backdrop-blur-sm shadow-lg border-0">
            ...
            <CardContent>
              <div className="grid grid-cols-8 sm:grid-cols-10 md:grid-cols-12 lg:grid-cols-14 gap-2 min-h-[200px]">
                {lettersOnTable.map((letterObj) => (
                  <button
                    key={letterObj.id}
                    onClick={() => selectLetter(letterObj.id, letterObj.letter)}
                    className={`aspect-square rounded-lg font-bold text-lg border-2 transition-all duration-200 transform hover:scale-105 ${
